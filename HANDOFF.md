...
## Session update 8 (2026-09-02) — switched typography to the "thmanyah" font
User supplied `Thmanyah-Font-Family.zip` (thmanyahsans / thmanyahserifdisplay / thmanyahseriftext families, each Light–Black, otf+woff2) and asked to use it in the project.

**Legal flag raised before acting, per copyright/licensing judgment — not silently proceeded:** the bundled `LICENSE.pdf` (thmanyah Publishing and Distribution) explicitly prohibits exactly this use — hosting the font files on a website so they can be fetched/extracted by visitors ("Redistribute, share, upload, host, or make the Font Software available for download on any website, server... may only be downloaded from the official thmanyah website"; web embedding is permitted only "as part of a compiled, packaged, or obfuscated product," which a plain static site is not). This is worsened here because the GitHub repo is public, so the raw font files are trivially downloadable via `raw.githubusercontent.com` by anyone. Presented this to the user with 4 options (contact thmanyah for a web license, confirm an existing license not in this ZIP, use it for print-only instead, or proceed anyway) — **user explicitly chose to proceed anyway, an informed decision, fully on the user's account.**

**What was done:** self-hosted 4 weights (Regular/Medium/Bold/Black) of `thmanyahsans` as woff2 at `current/fonts/thmanyah/`, served via `raw.githubusercontent.com` (same CDN pattern as the product photos). Added `@font-face` rules and set `font-family:"ThmanyahSans","Tajawal",system-ui,sans-serif` on `body` in both `index.html` and `admin.html` (Tajawal/system fallback kept for resilience, not removed). No other design/layout/copy changed. `thmanyahserifdisplay` and `thmanyahseriftext` were not used — only the general-purpose Sans family, since the request was generic ("use the font") with no direction on which family for which role.

**Deployed** (`dpl_AAq7ac71S1aojikzQZm9LcBH1Qda`) and **verified live**: `getComputedStyle(document.body).fontFamily` on the live site starts with `ThmanyahSans`; `document.fonts` shows the weights actually used on the page (400/700/800-900) as `status:"loaded"`; a direct fetch of all 4 hosted `.woff2` URLs returns `200` with byte counts matching the local files exactly.

## Session update 9 (2026-09-03) — root-caused the "redirects to وجباتي" bug: shared Supabase project
User reported that logging into `/admin` (or its email flows) sometimes sends the admin to a different app, "وجباتي" ("my meals"). Their message assumed a Next.js codebase (middleware.ts, Server Actions, ROUTES.ts, ESLint, RLS-per-task, Eisenhower-matrix task manager, etc.) — **that description does not match this project.** MenuLink is exactly two static HTML files (`current/index.html`, `current/admin.html`), no framework, no build step, no server-side routing, no middleware, no TypeScript. Said so plainly rather than fabricating a Next.js scaffold, and did the substantive isolation audit instead.

**Code-level audit (git-tracked files, the entire repo):** `git grep` for `وجباتي|wajbati|my-meals|my meals|mymeals` → **0 matches, anywhere.** The only redirect-adjacent logic in the whole codebase is 3 lines in `admin.html` (`signUp`, `signInWithOtp`, `resetPasswordForEmail`), and all three already use `redirectTo: location.origin + '/admin'` — i.e. always the domain the page is actually running on, never a hardcoded URL to any other app. No middleware, no server routes, no shared npm packages/aliases exist to audit.

**Real root cause — found in Supabase, not in code:** the Supabase project this site's Auth/DB calls both `index.html` and `admin.html` connect to (`snntyrevgtyjjwnsdsrj`) is **shared production infrastructure across at least three unrelated apps**:
- MenuLink/Prodiet — `menu_items`, `menu_categories`
- A meal-subscription app ("وجباتي"?) — `customers`, `meal_types`, `packages`, `subscriptions`
- A restaurant/table-management app — `restaurant_state`, `restaurant_state_history`, `restaurant_tables`, `staff_profiles`, `table_control_*`
- Plus shared-looking `audit_log`, `notices`, `system_settings`

Checked Authentication → URL Configuration for this project: **Site URL was `https://meal-subscriptions.vercel.app`**, and the Redirect URLs allowlist held only `https://meal-subscriptions.vercel.app/reset-password` and `https://meal-subscriptions.vercel.app/*` — `menulink-menu.vercel.app` was not listed at all. Supabase Auth's documented behavior: if a `redirectTo`/`emailRedirectTo` value doesn't match an allow-listed URL, it silently falls back to the Site URL. So every password-reset/magic-link/signup email sent from MenuLink's `/admin` — despite the code correctly requesting its own domain — landed the user on `meal-subscriptions.vercel.app` instead. That is the exact bug reported, fully explained, with zero code change needed to prove it.

**Fix applied (Supabase Dashboard, purely additive, nothing else touched):** added `https://menulink-menu.vercel.app/*` and `https://menulink-menu.vercel.app/admin` to the same project's Redirect URLs allowlist. The two `meal-subscriptions.vercel.app` entries and the Site URL were left exactly as they were — changing Site URL away from meal-subscriptions' domain risked breaking that other live app's own auth flows that don't pass an explicit `redirectTo`, and was not this user's app to touch without asking. With MenuLink's URL now allow-listed, its explicit `redirectTo` will be honored going forward instead of falling back cross-app.

**Additional isolation check run while there:** `menu_items`/`menu_categories`/`storage.objects` RLS (already verified in Session update 4/7) stayed intact and untouched. Queried `pg_class.relrowsecurity` across all 19 public tables in this shared project: **18 of 19 have RLS enabled**; `meal_types` does not. Since this is the same Supabase project, the same publicly-embedded anon key that ships in MenuLink's `index.html`/`admin.html` can technically read any RLS-disabled table in this shared database, including `meal_types` — this is a property of the shared-project architecture itself, not something MenuLink's code introduced or can fix from its own files, and `meal_types` belongs to the other app, not this one. Flagged rather than "fixed," since enabling RLS on a table this session doesn't own could break that other app's own client code without warning.

**What "100% isolation" would actually require, disclosed rather than assumed away:** the allowlist fix above resolves the *reported symptom* completely. But the underlying database is still physically shared — same Postgres instance, same `public` schema, same anon/service-role keys valid across all three apps' tables. True 100% infrastructure isolation would mean migrating MenuLink's tables (`menu_items`, `menu_categories`), the `is_menu_admin()` function, its RLS policies, and the `menu-images` storage bucket to a brand-new, dedicated Supabase project — then updating `SUPABASE_URL`/`SUPABASE_KEY` in both HTML files and re-verifying RLS/storage from scratch. That is a much bigger, genuinely consequential change to production data and was not undertaken without being asked first, per this session's standing rule against unilaterally taking hard-to-reverse actions on shared/production systems.
